...
function _buildHtml(maxLines, fontUri) {
    ...
    const jsLines = [
        ...
        'function makeTb(lines,level){',
        '  var last=parseLine(lines[lines.length-1]);',
        '  var lv=level||last.level||"ERROR";',
        '  var summary=esc((last.msg||lines[lines.length-1]).slice(0,120));',
        '  var hdrColor=lv==="CRITICAL"?"#c0392b":lv==="WARNING"?"#cca700":"#f48771";',
        ...
        '  var d=document.createElement("div");',
        '  d.className="tbg";',
        '  d.setAttribute("data-level",lv);',
        '  d.innerHTML="<div class=\\"tbh\\" style=\\"color:"+hdrColor+"\\" onclick=\\"toggleTb(this)\\"><span class=\\"tbt\\">&#9658;</span><span>"+summary+"</span></div><div class=\\"tbb\\">"+bodyHtml+"</div>";',
        '  return d;',
        '}',
        ...
        '    // Group traceback blocks AND multi-line warnings/errors (continuation lines)',
        '    if(p.msg.indexOf("Traceback (most recent call last)")!==-1){',
        '      var tb=[lines[i]];i++;',
        '      while(i<lines.length&&!lines[i].match(/^\\d{4}-\\d{2}-\\d{2}/)){tb.push(lines[i]);i++;}',
        '      out.push({type:"tb",lines:tb,level:"ERROR"});',
        '    }else if((p.level==="WARNING"||p.level==="ERROR"||p.level==="CRITICAL")&&i+1<lines.length&&!lines[i+1].match(/^\\d{4}-\\d{2}-\\d{2}/)){',
        '      // Multi-line warning/error: group continuation lines',
        '      var tb=[lines[i]];i++;',
        '      while(i<lines.length&&!lines[i].match(/^\\d{4}-\\d{2}-\\d{2}/)){tb.push(lines[i]);i++;}',
        '      if(tb.length>1){out.push({type:"tb",lines:tb,level:p.level});}',
        '      else{out.push({type:"ln",p:p});}',
        ...
        '    var g=groups[i];',
        '    if(g.type==="tb"){',
        '      var el=makeTb(g.lines,g.level);',
        '      // Hide group if its level is filtered out — but keep it in DOM so expand works',
        '      var lv=g.level||"ERROR";',
        '      var p={level:lv,raw:g.lines[0]||""};',
        '      if(!mf(p)||!ms(p))el.style.display="none";',
        '      frag.appendChild(el);',
        ...
        '  var groups=groupTb(lines);',
        '  var frag=renderGroups(groups);',
